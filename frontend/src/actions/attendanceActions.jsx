...
export const postAttendance = (attendance) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(`http://localhost:5500/attendance/`, attendance, config);
    dispatch({
...
export const getAnalysisByDate = (date) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(
      `http://localhost:5500/attendance/getAnalysis`,
      { date: date },
      config
    );
...
export const deleteAttendanceByDate = (days) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.delete(`http://localhost:5500/attendance/${days}`, config);
