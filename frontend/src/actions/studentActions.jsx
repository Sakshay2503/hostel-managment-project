...
export const listStudents = (keyword = "", pageNumber = "") => async (
  ...
  try {
    ...
    const { data } = await axios.get(
      `http://localhost:5500/student/all?keyword=${keyword}&pageNumber=${pageNumber}`,
      config
    );
    dispatch({
...
export const addStudent = (student) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.post(`http://localhost:5500/student/addStudent`, student, config);
...
export const getStudentDetails = (id) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`http://localhost:5500/student/${id}`, config);
...
export const deleteStudent = (id) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.delete(`http://localhost:5500/student/${id}`, config);
...
export const getStudentsByRoomNo = (roomNo) => async (dispatch, getState) => {
  try {
    ...
    const { data } = await axios.get(`http://localhost:5500/student/room/${roomNo}`, config);
    dispatch({
